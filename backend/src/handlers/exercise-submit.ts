...
import { submitCodeSchema } from "../schemas/exercise.schema";
import { submitCode } from "../services/exercise.service";
...
export const handler = async (event: APIGatewayProxyEventV2) => {
  try {
    ...
    const input = submitCodeSchema.parse(body);
    const result = await submitCode(auth.user!.cognitoId, slug, input);
